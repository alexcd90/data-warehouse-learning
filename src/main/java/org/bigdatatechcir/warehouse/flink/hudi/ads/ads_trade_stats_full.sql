...
CREATE TABLE IF NOT EXISTS hudi_ads.ads_trade_stats_full(
...
CREATE TEMPORARY VIEW tmp_ads_trade_stats_order_nd_stats AS
    ...
    LEFT JOIN hudi_dws.dws_trade_user_order_nd_full /*+ OPTIONS('read.streaming.enabled' = 'false') */ o
        ON o.k1 = '${pdate}'
...
CREATE TEMPORARY VIEW tmp_ads_trade_stats_refund_nd_stats AS
    ...
    FROM tmp_ads_trade_stats_recent_days r
    LEFT JOIN hudi_dws.dws_trade_user_order_refund_nd_full /*+ OPTIONS('read.streaming.enabled' = 'false') */ f
        ON f.k1 = '${pdate}'
    GROUP BY r.recent_days
;

INSERT INTO hudi_ads.ads_trade_stats_full(
    dt,
    recent_days,
    order_total_amount,
    order_count,
