...
SET 'pipeline.name' = 'hudi_dws_stream_dws_trade_coupon_order_nd_full';
...
CREATE TABLE IF NOT EXISTS hudi_dws_stream.dws_trade_coupon_order_nd_full(
...
CREATE TEMPORARY VIEW tmp_dws_trade_coupon_order_nd_coupon_union AS
    ...
    FROM hudi_dim.dim_coupon_full /*+ OPTIONS('read.streaming.enabled' = 'true') */
    ...
    FROM hudi_dwd_stream.dwd_tool_coupon_order_full /*+ OPTIONS('read.streaming.enabled' = 'true') */ c
    JOIN hudi_dwd_stream.dwd_trade_order_detail_full /*+ OPTIONS('read.streaming.enabled' = 'true') */ od
...
INSERT INTO hudi_dws_stream.dws_trade_coupon_order_nd_full(
    coupon_id,
    k1,
    coupon_name,
    coupon_type_code,
