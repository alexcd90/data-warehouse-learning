...
SET 'pipeline.name' = 'hudi_ads_stream_ads_user_action_full';
...
CREATE TABLE IF NOT EXISTS hudi_ads_stream.ads_user_action_full(
...
JOIN hudi_dws_stream.dws_traffic_page_visitor_page_view_nd_full /*+ OPTIONS('read.streaming.enabled' = 'true') */ pv
    ON p.recent_days IN (7, 30)
   AND pv.k1 = DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd')
...
FROM tmp_ads_user_action_recent_days p
JOIN hudi_dws_stream.dws_trade_user_cart_add_nd_full /*+ OPTIONS('read.streaming.enabled' = 'true') */ c
    ON p.recent_days IN (7, 30)
   AND c.k1 = DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd')
GROUP BY p.recent_days
;
...
FROM tmp_ads_user_action_recent_days p
JOIN hudi_dws_stream.dws_trade_user_order_nd_full /*+ OPTIONS('read.streaming.enabled' = 'true') */ o
    ON p.recent_days IN (7, 30)
   AND o.k1 = DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd')
GROUP BY p.recent_days
;
...
FROM tmp_ads_user_action_recent_days p
JOIN hudi_dws_stream.dws_trade_user_payment_nd_full /*+ OPTIONS('read.streaming.enabled' = 'true') */ pay
    ON p.recent_days IN (7, 30)
   AND pay.k1 = DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd')
GROUP BY p.recent_days
;

INSERT INTO hudi_ads_stream.ads_user_action_full(
    dt,
    recent_days,
    home_count,
    good_detail_count,
