...
CREATE TABLE IF NOT EXISTS hudi_ads.ads_sku_association_analysis_full(
...
CREATE TEMPORARY TABLE tmp_ads_sku_association_order_detail_snapshot
WITH (
    'read.streaming.enabled' = 'false'
)
LIKE hudi_dwd.dwd_trade_order_detail_full;

CREATE TEMPORARY TABLE tmp_ads_sku_association_dim_sku_snapshot
WITH (
    'read.streaming.enabled' = 'false'
)
...
WHERE CAST(k1 AS DATE) >= TIMESTAMPADD(DAY, -90, CAST('${pdate}' AS DATE))
  AND CAST(create_time AS DATE) >= TIMESTAMPADD(DAY, -CAST(d.recent_days - 1 AS INT), CAST('${pdate}' AS DATE))
  AND CAST(create_time AS DATE) <= CAST('${pdate}' AS DATE);
...
INSERT INTO hudi_ads.ads_sku_association_analysis_full(
    dt,
    recent_days,
    source_sku_id,
    source_sku_name,
