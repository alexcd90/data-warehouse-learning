...
CREATE TABLE IF NOT EXISTS hudi_ads.ads_trade_stats_by_tm_category_full(
...
INSERT INTO hudi_ads.ads_trade_stats_by_tm_category_full (dt, recent_days, tm_id, tm_name, category1_id, category1_name, 
    ...
    (
        ...
        (
            ...
            SELECT
                ...
                hudi_dim.tm_id,
                hudi_dim.tm_name,
                hudi_dim.category1_id,
                hudi_dim.category1_name,
                hudi_dwd.split_total_amount
            FROM 
            (
                -- 获取近7天和30天的订单明细数据
                SELECT
            ...
            ON hudi_dwd.sku_id = hudi_dim.id               -- 关联订单明细和商品维度
            WHERE date_format(hudi_dwd.create_time, 'yyyy-MM-dd') >= date_sub(CAST('${pdate}' AS DATE), d.recent_days - 1)  -- 按统计周期筛选
            AND date_format(hudi_dwd.create_time, 'yyyy-MM-dd') <= CAST('${pdate}' AS DATE)   -- 最大日期为调度日期
        ) detail
    ) stats
