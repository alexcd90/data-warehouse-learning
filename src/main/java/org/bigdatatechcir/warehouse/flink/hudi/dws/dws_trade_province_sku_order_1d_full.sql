...
CREATE TABLE IF NOT EXISTS hudi_dws.dws_trade_province_sku_order_1d_full(
...
INSERT INTO hudi_dws.dws_trade_province_sku_order_1d_full(
    province_id,
    sku_id,
    k1,
    province_name,
...
LEFT JOIN hudi_dim.dim_province_full /*+ OPTIONS('read.streaming.enabled' = 'false') */ p
    ON od.province_id = p.id
LEFT JOIN hudi_dim.dim_sku_full /*+ OPTIONS('read.streaming.enabled' = 'false') */ sku
    ON od.sku_id = sku.id
   AND od.k1 = sku.k1;
