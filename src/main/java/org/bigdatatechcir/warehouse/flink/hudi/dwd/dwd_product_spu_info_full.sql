...
CREATE TABLE IF NOT EXISTS hudi_dwd.dwd_product_spu_info_full(
...
INSERT INTO hudi_dwd.dwd_product_spu_info_full(
    id,
    k1,
    spu_id,
    spu_name,
...
WITH img AS (
    ...
    FROM hudi_ods.ods_spu_image_full
    GROUP BY spu_id
),
sale_attr AS (
    SELECT
    ...
    FROM hudi_ods.ods_spu_sale_attr_full sa
    LEFT JOIN hudi_ods.ods_spu_sale_attr_value_full sav
...
poster AS (
    ...
    FROM hudi_ods.ods_spu_poster_full
    WHERE is_deleted = 0
    GROUP BY spu_id
)
SELECT
...
FROM hudi_ods.ods_spu_info_full sp
LEFT JOIN hudi_ods.ods_base_trademark_full tm
    ON sp.tm_id = tm.id
LEFT JOIN hudi_ods.ods_base_category3_full c3
