...
SET 'pipeline.name' = 'hudi_ads_stream_ads_province_conversion_analysis_full';
...
CREATE TABLE IF NOT EXISTS hudi_ads_stream.ads_province_conversion_analysis_full(
...
CREATE TEMPORARY TABLE tmp_ads_province_conversion_order_detail_snapshot
WITH (
    'read.streaming.enabled' = 'false'
)
LIKE hudi_dwd_stream.dwd_trade_order_detail_full;

CREATE TEMPORARY TABLE tmp_ads_province_conversion_pay_detail_snapshot
WITH (
    'read.streaming.enabled' = 'false'
)
LIKE hudi_dwd_stream.dwd_trade_pay_detail_suc_full;

CREATE TEMPORARY VIEW tmp_ads_province_conversion_scored AS
SELECT
    rd.recent_days,
    ...
    CAST(COALESCE(ord.order_amount, 0) AS DECIMAL(16, 2)) AS order_amount_for_rank
FROM hudi_dim.dim_province_full /*+ OPTIONS('read.streaming.enabled' = 'true') */ p
CROSS JOIN (
...
LEFT JOIN (
    ...
    FROM tmp_ads_province_conversion_order_detail_snapshot od
    CROSS JOIN (
        SELECT CAST(1 AS BIGINT) AS recent_days
        UNION ALL
        SELECT CAST(7 AS BIGINT)
    ...
    WHERE CAST(od.k1 AS DATE) >= TIMESTAMPADD(DAY, -30, CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE))
...
LEFT JOIN (
    ...
    FROM tmp_ads_province_conversion_pay_detail_snapshot pd
    CROSS JOIN (
        SELECT CAST(1 AS BIGINT) AS recent_days
        UNION ALL
        SELECT CAST(7 AS BIGINT)
    ...
    WHERE CAST(pd.k1 AS DATE) >= TIMESTAMPADD(DAY, -30, CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE))
      AND CAST(pd.callback_time AS DATE) >= TIMESTAMPADD(DAY, -CAST(d.recent_days - 1 AS INT), CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE))
      AND CAST(pd.callback_time AS DATE) <= CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE)
...
LEFT JOIN (
    ...
    FROM tmp_ads_province_conversion_order_detail_snapshot ao
    CROSS JOIN (
        SELECT CAST(1 AS BIGINT) AS recent_days
        UNION ALL
        SELECT CAST(7 AS BIGINT)
    ...
    WHERE CAST(ao.k1 AS DATE) >= TIMESTAMPADD(DAY, -30, CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE))
      AND CAST(ao.create_time AS DATE) >= TIMESTAMPADD(DAY, -CAST(d.recent_days - 1 AS INT), CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE))
      AND CAST(ao.create_time AS DATE) <= CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE)
...
LEFT JOIN (
    ...
    FROM tmp_ads_province_conversion_pay_detail_snapshot ap
    CROSS JOIN (
        SELECT CAST(1 AS BIGINT) AS recent_days
        UNION ALL
        SELECT CAST(7 AS BIGINT)
    ...
    WHERE CAST(ap.k1 AS DATE) >= TIMESTAMPADD(DAY, -30, CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE))
      AND CAST(ap.callback_time AS DATE) >= TIMESTAMPADD(DAY, -CAST(d.recent_days - 1 AS INT), CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE))
      AND CAST(ap.callback_time AS DATE) <= CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE)
    GROUP BY d.recent_days
) all_pay
...
LEFT JOIN (
    ...
    FROM tmp_ads_province_conversion_order_detail_snapshot lw
    CROSS JOIN (
        SELECT CAST(1 AS BIGINT) AS recent_days
        UNION ALL
        SELECT CAST(7 AS BIGINT)
    ...
    WHERE CAST(lw.k1 AS DATE) >= TIMESTAMPADD(DAY, -30, CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE))
      AND CAST(lw.create_time AS DATE) >= TIMESTAMPADD(
            DAY,
            -CAST(d.recent_days - 1 AS INT),
            TIMESTAMPADD(DAY, -7, CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE))
          )
      AND CAST(lw.create_time AS DATE) <= TIMESTAMPADD(DAY, -7, CAST(DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd') AS DATE))
...
CREATE TEMPORARY VIEW tmp_ads_province_conversion_ranked AS
SELECT
    cur.recent_days,
    cur.province_id,
    cur.province_name,
    ...
    cur.region_name,
    cur.visitor_count,
    cur.product_view_count,
    cur.cart_count,
    cur.order_count,
    cur.payment_count,
    cur.view_to_cart_rate,
    cur.cart_to_order_rate,
    cur.order_to_payment_rate,
    cur.overall_conversion_rate,
    cur.average_order_amount,
    cur.user_penetration_rate,
    cur.gmv_contribution_rate,
    cur.wow_change_rate,
    CAST(COUNT(higher.province_id) + 1 AS BIGINT) AS regional_rank
FROM tmp_ads_province_conversion_scored cur
LEFT JOIN tmp_ads_province_conversion_scored higher
    ON cur.recent_days = higher.recent_days
    AND (
        higher.order_amount_for_rank > cur.order_amount_for_rank
        OR (
            higher.order_amount_for_rank = cur.order_amount_for_rank
            AND CAST(higher.province_id AS BIGINT) < CAST(cur.province_id AS BIGINT)
        )
    )
GROUP BY
    cur.recent_days,
    ...
    cur.gmv_contribution_rate,
    cur.wow_change_rate,
    cur.order_amount_for_rank
;

INSERT INTO hudi_ads_stream.ads_province_conversion_analysis_full(
    dt,
    recent_days,
    province_id,
    province_name,
...
SELECT
    ...
    wow_change_rate,
    regional_rank
FROM tmp_ads_province_conversion_ranked
;
