...
SET 'pipeline.name' = 'hudi_dim_stream_dim_coupon_full';
...
CREATE TABLE IF NOT EXISTS hudi_dim_stream.dim_coupon_full(
...
insert into hudi_dim_stream.dim_coupon_full(
    id,
    k1,
    coupon_name,
    coupon_type_code,
...
from
    (
        select
            ...
            operate_time,
            expire_time
        from hudi_ods.ods_coupon_info_full
    ) ci
    left join
    (
        select
