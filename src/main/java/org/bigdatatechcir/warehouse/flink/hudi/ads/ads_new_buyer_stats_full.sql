...
CREATE TABLE IF NOT EXISTS hudi_ads.ads_new_buyer_stats_full(
...
LEFT JOIN hudi_dws.dws_trade_user_order_td_full /*+ OPTIONS('read.streaming.enabled' = 'false') */ o
    ON o.k1 = '${pdate}'
...
FROM tmp_ads_new_buyer_stats_recent_days p
CROSS JOIN tmp_ads_new_buyer_stats_current_date_param cp
LEFT JOIN hudi_dws.dws_trade_user_payment_td_full /*+ OPTIONS('read.streaming.enabled' = 'false') */ pay
    ON pay.k1 = '${pdate}'
GROUP BY p.recent_days
;

INSERT INTO hudi_ads.ads_new_buyer_stats_full(
    dt,
    recent_days,
