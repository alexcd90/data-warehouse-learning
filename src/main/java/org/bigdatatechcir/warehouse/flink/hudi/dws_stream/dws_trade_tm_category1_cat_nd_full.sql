...
SET 'pipeline.name' = 'hudi_dws_stream_dws_trade_tm_category1_cat_nd_full';
...
CREATE TABLE IF NOT EXISTS hudi_dws_stream.dws_trade_tm_category1_cat_nd_full(
...
latest_sku AS (
    SELECT s.*
    FROM hudi_dim.dim_sku_full /*+ OPTIONS('read.streaming.enabled' = 'true') */ s
...
detail_by_day_sku AS (
    ...
    FROM hudi_dwd_stream.dwd_trade_order_detail_full /*+ OPTIONS('read.streaming.enabled' = 'true') */ od
...
INSERT INTO hudi_dws_stream.dws_trade_tm_category1_cat_nd_full(
    tm_id,
    category1_id,
    k1,
    tm_name,
