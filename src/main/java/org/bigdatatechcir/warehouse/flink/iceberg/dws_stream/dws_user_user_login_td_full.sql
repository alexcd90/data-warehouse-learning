...
SET 'pipeline.name' = 'iceberg_dws_stream_dws_user_user_login_td_full';
...
CREATE TABLE IF NOT EXISTS iceberg_dws_stream.dws_user_user_login_td_full(
...
CREATE TEMPORARY VIEW tmp_dws_user_user_login_td_user_dim AS
    SELECT
        id AS user_id,
        create_time
    FROM iceberg_dim_stream.dim_user_zip_full
...
CREATE TEMPORARY VIEW tmp_dws_user_user_login_td_login_agg AS
    ...
    FROM iceberg_dwd_stream.dwd_user_login_full
    CROSS JOIN tmp_dws_user_user_login_td_current_date_param cp
    WHERE CAST(k1 AS DATE) <= cp.cur_date
    GROUP BY CAST(user_id AS BIGINT)
;

INSERT INTO iceberg_dws_stream.dws_user_user_login_td_full(
    user_id,
    k1,
    login_date_last,
    login_count_td
