...
SET 'pipeline.name' = 'iceberg_dwd_stream_dwd_user_login_full';
...
CREATE TABLE IF NOT EXISTS iceberg_dwd_stream.dwd_user_login_full(
...
CREATE TEMPORARY VIEW iceberg_dwd_user_login_login_events AS
SELECT
    common_uid user_id,
    common_ch channel,
    common_ar area_code,
...
CREATE TEMPORARY VIEW iceberg_dwd_user_login_first_daily_login AS
SELECT
    user_id,
    date_id,
    date_id AS k1,
...
FROM iceberg_dwd_user_login_login_events
WHERE ts IS NOT NULL
GROUP BY user_id, date_id
;

INSERT INTO iceberg_dwd_stream.dwd_user_login_full(
    k1,
    user_id,
    date_id,
    login_time,
...
FROM iceberg_dwd_user_login_first_daily_login login
LEFT JOIN
(
    SELECT
        id AS province_id,
