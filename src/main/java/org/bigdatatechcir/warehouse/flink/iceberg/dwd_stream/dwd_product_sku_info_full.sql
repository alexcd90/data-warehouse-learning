...
SET 'pipeline.name' = 'iceberg_dwd_stream_dwd_product_sku_info_full';
...
CREATE TABLE IF NOT EXISTS iceberg_dwd_stream.dwd_product_sku_info_full(
...
WITH attr AS (
    ...
    FROM iceberg_ods.ods_sku_attr_value_full
    GROUP BY sku_id
),
sale_attr AS (
    SELECT
    ...
    FROM iceberg_ods.ods_sku_sale_attr_value_full
    GROUP BY sku_id
),
cost AS (
    SELECT
    ...
    FROM iceberg_ods.ods_financial_sku_cost_full
    WHERE is_lastest = '1'
    GROUP BY sku_id
)
SELECT
...
FROM iceberg_ods.ods_sku_info_full s
LEFT JOIN iceberg_ods.ods_base_trademark_full tm
    ON s.tm_id = tm.id
LEFT JOIN iceberg_ods.ods_base_category3_full c3
...
LEFT JOIN cost
    ON s.id = cost.sku_id
WHERE s.k1 = DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd');

INSERT INTO iceberg_dwd_stream.dwd_product_sku_info_full(
    id,
    k1,
    sku_id,
    spu_id,
