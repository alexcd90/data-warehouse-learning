...
SET 'pipeline.name' = 'iceberg_ads_stream_ads_user_action_full';
...
CREATE TABLE IF NOT EXISTS iceberg_ads_stream.ads_user_action_full(
...
FROM iceberg_dws_stream.dws_traffic_page_visitor_page_view_1d_full
...
JOIN iceberg_dws_stream.dws_traffic_page_visitor_page_view_nd_full pv
...
SELECT
    CAST(1 AS BIGINT) AS recent_days,
    COUNT(*) AS cart_count
FROM iceberg_dws_stream.dws_trade_user_cart_add_1d_full
WHERE k1 = DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd')
UNION ALL
SELECT
    p.recent_days,
...
FROM tmp_ads_user_action_recent_days p
JOIN iceberg_dws_stream.dws_trade_user_cart_add_nd_full c
    ON p.recent_days IN (7, 30)
...
FROM tmp_ads_user_action_recent_days p
JOIN iceberg_dws_stream.dws_trade_user_order_nd_full o
    ON p.recent_days IN (7, 30)
...
FROM tmp_ads_user_action_recent_days p
JOIN iceberg_dws_stream.dws_trade_user_payment_nd_full pay
    ON p.recent_days IN (7, 30)
   AND pay.k1 = DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd')
GROUP BY p.recent_days
;

INSERT INTO iceberg_ads_stream.ads_user_action_full(
    dt,
    recent_days,
    home_count,
    good_detail_count,
