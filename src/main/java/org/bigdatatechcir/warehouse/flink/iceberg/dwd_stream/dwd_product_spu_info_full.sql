...
SET 'pipeline.name' = 'iceberg_dwd_stream_dwd_product_spu_info_full';
...
CREATE TABLE IF NOT EXISTS iceberg_dwd_stream.dwd_product_spu_info_full(
...
FROM iceberg_ods.ods_spu_image_full
GROUP BY spu_id;
...
FROM iceberg_ods.ods_spu_sale_attr_full sa
LEFT JOIN iceberg_ods.ods_spu_sale_attr_value_full sav
...
FROM iceberg_ods.ods_spu_poster_full
WHERE is_deleted = 0
GROUP BY spu_id;

INSERT INTO iceberg_dwd_stream.dwd_product_spu_info_full(
    id,
    k1,
    spu_id,
    spu_name,
...
FROM iceberg_ods.ods_spu_info_full sp
