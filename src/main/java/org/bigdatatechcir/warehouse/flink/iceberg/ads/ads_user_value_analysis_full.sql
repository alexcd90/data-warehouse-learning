...
CREATE TABLE IF NOT EXISTS iceberg_ads.ads_user_value_analysis_full(
...
FROM iceberg_dim.dim_user_zip_full d
JOIN (
...
FROM iceberg_ads.ads_user_value_analysis_full p
...
FROM iceberg_dws.dws_trade_user_order_td_full o
LEFT JOIN iceberg_dws.dws_user_user_login_td_full l
...
INSERT INTO iceberg_ads.ads_user_value_analysis_full /*+ OPTIONS('upsert-enabled' = 'true') */(
    dt,
    user_id,
    order_count_td,
    order_amount_td,
