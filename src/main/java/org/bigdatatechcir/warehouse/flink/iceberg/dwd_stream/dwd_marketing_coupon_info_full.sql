...
SET 'pipeline.name' = 'iceberg_dwd_stream_dwd_marketing_coupon_info_full';
...
CREATE TABLE IF NOT EXISTS iceberg_dwd_stream.dwd_marketing_coupon_info_full(
...
SELECT
    ...
    ci.limit_num
FROM iceberg_ods.ods_coupon_info_full ci
LEFT JOIN coupon_range cr
    ON ci.id = cr.coupon_id
WHERE ci.k1 = DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd');

INSERT INTO iceberg_dwd_stream.dwd_marketing_coupon_info_full(
    id,
    k1,
    coupon_id,
    coupon_name,
