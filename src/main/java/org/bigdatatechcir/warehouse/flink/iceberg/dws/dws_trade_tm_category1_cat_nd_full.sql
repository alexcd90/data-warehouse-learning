...
CREATE TABLE IF NOT EXISTS iceberg_dws.dws_trade_tm_category1_cat_nd_full(
...
max_sku_date AS (
    SELECT MAX(k1) AS max_k1 FROM iceberg_dim.dim_sku_full
),
latest_sku AS (
    SELECT s.*
    FROM iceberg_dim.dim_sku_full s
...
INSERT INTO iceberg_dws.dws_trade_tm_category1_cat_nd_full /*+ OPTIONS('upsert-enabled' = 'true') */(
    tm_id,
    category1_id,
    k1,
    tm_name,
