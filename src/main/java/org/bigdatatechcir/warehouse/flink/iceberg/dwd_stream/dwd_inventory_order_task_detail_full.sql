...
SET 'pipeline.name' = 'iceberg_dwd_stream_dwd_inventory_order_task_detail_full';
...
CREATE TABLE IF NOT EXISTS iceberg_dwd_stream.dwd_inventory_order_task_detail_full(
...
INSERT INTO iceberg_dwd_stream.dwd_inventory_order_task_detail_full(
    id,
    k1,
    task_id,
    order_id,
...
FROM iceberg_ods.ods_ware_order_task_detail_full wotd
LEFT JOIN
(
    SELECT
        id,
...
(
    SELECT
        ...
        oi.user_id
    FROM iceberg_ods.ods_order_detail_full od
    LEFT JOIN iceberg_ods.ods_order_info_full oi
