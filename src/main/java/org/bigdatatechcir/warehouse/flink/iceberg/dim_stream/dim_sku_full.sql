...
SET 'pipeline.name' = 'iceberg_dim_stream_dim_sku_full';
...
CREATE TABLE IF NOT EXISTS iceberg_dim_stream.dim_sku_full(
...
INSERT INTO iceberg_dim_stream.dim_sku_full(
    id,
    k1,
    price,
    sku_name,
...
from
    (
        select
            ...
            sku_default_img,
            create_time
        from iceberg_ods.ods_sku_info_full
    ) s
    left join
    (
        select
            id,
            spu_name
        from iceberg_ods.ods_spu_info_full
    ) sp
    on s.spu_id = sp.id
    left join
    (
        select
            ...
            name,
            category2_id
        from iceberg_ods.ods_base_category3_full
    ) c3
    ...
    (
        select
            ...
            name,
            category1_id
        from iceberg_ods.ods_base_category2_full
    ) c2
    ...
    (
        select
            id,
            tm_name
        from iceberg_ods.ods_base_trademark_full
    ) tm
    on s.tm_id = tm.id
    left join
    (
