...
INSERT INTO ads.ads_province_conversion_analysis_full(
    dt,
    recent_days,
    province_id,
    province_name,
...
LEFT JOIN (
    ...
    FROM dwd.dwd_trade_pay_detail_suc_full pd
    CROSS JOIN (
        SELECT CAST(1 AS BIGINT) AS recent_days
        UNION ALL
        SELECT CAST(7 AS BIGINT)
...
LEFT JOIN (
    ...
    FROM dwd.dwd_trade_order_detail_full ao
    CROSS JOIN (
        SELECT CAST(1 AS BIGINT) AS recent_days
        UNION ALL
        SELECT CAST(7 AS BIGINT)
...
LEFT JOIN (
    ...
    FROM dwd.dwd_trade_pay_detail_suc_full ap
    CROSS JOIN (
        SELECT CAST(1 AS BIGINT) AS recent_days
        UNION ALL
        SELECT CAST(7 AS BIGINT)
...
LEFT JOIN (
    ...
    FROM dwd.dwd_trade_order_detail_full lw
    CROSS JOIN (
        SELECT CAST(1 AS BIGINT) AS recent_days
        UNION ALL
        SELECT CAST(7 AS BIGINT)
