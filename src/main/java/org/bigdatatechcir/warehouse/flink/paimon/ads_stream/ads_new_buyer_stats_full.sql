...
SET 'pipeline.name' = 'paimon_ads_stream_ads_new_buyer_stats_full';
...
CREATE TABLE IF NOT EXISTS ads_stream.ads_new_buyer_stats_full(
...
LEFT JOIN dws_stream.dws_trade_user_order_td_full o
...
FROM tmp_ads_new_buyer_stats_recent_days p
CROSS JOIN tmp_ads_new_buyer_stats_current_date_param cp
LEFT JOIN dws_stream.dws_trade_user_payment_td_full pay
    ON pay.k1 = DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd')
GROUP BY p.recent_days
;

INSERT INTO ads_stream.ads_new_buyer_stats_full(
    dt,
    recent_days,
