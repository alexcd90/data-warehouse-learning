...
SET 'pipeline.name' = 'paimon_ads_stream_ads_trade_stats_full';
...
CREATE TABLE IF NOT EXISTS ads_stream.ads_trade_stats_full(
...
CREATE TEMPORARY VIEW tmp_ads_trade_stats_order_nd_stats AS
    ...
    LEFT JOIN dws_stream.dws_trade_user_order_nd_full o
...
CREATE TEMPORARY VIEW tmp_ads_trade_stats_refund_nd_stats AS
    ...
    FROM tmp_ads_trade_stats_recent_days r
    LEFT JOIN dws_stream.dws_trade_user_order_refund_nd_full f
        ON f.k1 = DATE_FORMAT(CURRENT_TIMESTAMP, 'yyyy-MM-dd')
    GROUP BY r.recent_days
;

INSERT INTO ads_stream.ads_trade_stats_full(
    dt,
    recent_days,
    order_total_amount,
    order_count,
