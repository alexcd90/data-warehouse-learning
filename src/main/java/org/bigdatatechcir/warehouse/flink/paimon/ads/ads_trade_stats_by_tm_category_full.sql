...
INSERT INTO ads.ads_trade_stats_by_tm_category_full (dt, recent_days, tm_id, tm_name, category1_id, category1_name, 
...
(
    ...
    (
        ...
        (
            ...
            WHERE date_format(dwd.create_time, 'yyyy-MM-dd') >= date_sub(CAST('${pdate}' AS DATE), d.recent_days - 1)  -- 按统计周期筛选
            AND date_format(dwd.create_time, 'yyyy-MM-dd') <= CAST('${pdate}' AS DATE)   -- 最大日期为调度日期
        ) detail
    ) stats
