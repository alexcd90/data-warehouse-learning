...
SET 'pipeline.name' = 'paimon_dwd_stream_dwd_inventory_stock_full';
...
CREATE TABLE IF NOT EXISTS dwd_stream.dwd_inventory_stock_full(
...
INSERT INTO dwd_stream.dwd_inventory_stock_full(
    id,
    k1,
    sku_id,
    sku_name,
...
FROM ods.ods_ware_sku_full ws
LEFT JOIN ods.ods_ware_info_full wi
    ON ws.warehouse_id = wi.id
LEFT JOIN ods.ods_sku_info_full si
    ON ws.sku_id = si.id;
