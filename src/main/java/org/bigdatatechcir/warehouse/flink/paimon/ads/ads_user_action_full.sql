...
CREATE TABLE IF NOT EXISTS ads.ads_user_action_full(
...
JOIN dws.dws_traffic_page_visitor_page_view_nd_full pv
...
FROM tmp_ads_user_action_recent_days p
JOIN dws.dws_trade_user_cart_add_nd_full c
    ON p.recent_days IN (7, 30)
   AND c.k1 = '${pdate}'
GROUP BY p.recent_days
;
...
FROM tmp_ads_user_action_recent_days p
JOIN dws.dws_trade_user_order_nd_full o
    ON p.recent_days IN (7, 30)
   AND o.k1 = '${pdate}'
GROUP BY p.recent_days
;
...
FROM tmp_ads_user_action_recent_days p
JOIN dws.dws_trade_user_payment_nd_full pay
    ON p.recent_days IN (7, 30)
   AND pay.k1 = '${pdate}'
GROUP BY p.recent_days
;

INSERT INTO ads.ads_user_action_full(
    dt,
    recent_days,
    home_count,
    good_detail_count,
