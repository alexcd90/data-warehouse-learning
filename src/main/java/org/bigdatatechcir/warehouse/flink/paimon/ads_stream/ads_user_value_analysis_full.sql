...
SET 'pipeline.name' = 'paimon_ads_stream_ads_user_value_analysis_full';
...
CREATE TABLE IF NOT EXISTS ads_stream.ads_user_value_analysis_full(
...
SELECT *
FROM dim_stream.dim_user_zip_full /*+ OPTIONS('read.streaming.enabled' = 'false') */
;

CREATE TEMPORARY VIEW tmp_ads_user_value_analysis_order_td_snapshot AS
SELECT *
FROM dws_stream.dws_trade_user_order_td_full /*+ OPTIONS('read.streaming.enabled' = 'false') */
;
...
SELECT *
FROM ads_stream.ads_user_value_analysis_full /*+ OPTIONS('read.streaming.enabled' = 'false') */
;
...
INSERT INTO ads_stream.ads_user_value_analysis_full(
    dt,
    user_id,
    order_count_td,
    order_amount_td,
